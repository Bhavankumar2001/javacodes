...
public class LotSizingService {
	...
	// @GetMapping(value="/vkc/filter")
		...
		public List<LotSizingPropertiesDAO> getlotsizing()
				throws UnirestException, JsonMappingException, JsonProcessingException {

			HttpResponse<String> jsonresponse = Utils.ApiCall(
					"https://" + Utils.port + "-" + APIConstants.masterReport.YY1_LOTSIZINGPROCEDURE,
					Utils.apiUserName, Utils.apiPassword);
			JsonNode getJsonValues = Utils.XmlToJsonConversion(jsonresponse.getBody());
			ObjectMapper objectMapper = new ObjectMapper();

			List<LotSizingDAO> entryNodesValues = objectMapper.reader()
					.forType(objectMapper.getTypeFactory().constructCollectionType(List.class,LotSizingDAO.class))
					.readValue(getJsonValues.toString());

			List<LotSizingPropertiesDAO> customerList = entryNodesValues.stream()
