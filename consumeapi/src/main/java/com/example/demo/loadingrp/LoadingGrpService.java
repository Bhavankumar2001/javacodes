...
@Service
public class LoadingGrpService {
	@Autowired
	UtilsService Utils;
	
	// @GetMapping(value="/vkc/filter")
		@Description(value = "Loading  API Consume")
		public List<LoadingGrpPropertiesDAO> getloadinggrp()
				throws UnirestException, JsonMappingException, JsonProcessingException {

			HttpResponse<String> jsonresponse = Utils.ApiCall(
					"https://" + Utils.port + "-" + APIConstants.masterReport.YY1_LOADINGGRP,
					Utils.apiUserName, Utils.apiPassword);
			JsonNode getJsonValues = Utils.XmlToJsonConversion(jsonresponse.getBody());
			ObjectMapper objectMapper = new ObjectMapper();

			List<LoadingGrpDAO> entryNodesValues = objectMapper.reader()
					.forType(objectMapper.getTypeFactory().constructCollectionType(List.class, LoadingGrpDAO.class))
					.readValue(getJsonValues.toString());

			List<LoadingGrpPropertiesDAO> customerList = entryNodesValues.stream()
