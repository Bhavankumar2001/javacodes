...
public class BackFlushService {
	...
	// @GetMapping(value="/vkc/filter")
		...
		public List<BackFlushPropertiesDAO> getbackflush()
				throws UnirestException, JsonMappingException, JsonProcessingException {

			HttpResponse<String> jsonresponse = Utils.ApiCall(
					"https://" + Utils.port + "-" + APIConstants.masterReport.YY1_BFLUSH,
					Utils.apiUserName, Utils.apiPassword);
			JsonNode getJsonValues = Utils.XmlToJsonConversion(jsonresponse.getBody());
			ObjectMapper objectMapper = new ObjectMapper();

			List<BackFlushDAO> entryNodesValues = objectMapper.reader()
					.forType(objectMapper.getTypeFactory().constructCollectionType(List.class, BackFlushDAO.class))
					.readValue(getJsonValues.toString());

			List<BackFlushPropertiesDAO> customerList = entryNodesValues.stream()
