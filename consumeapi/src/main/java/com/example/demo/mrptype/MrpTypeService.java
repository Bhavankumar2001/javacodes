...
@Service
public class MrpTypeService {
	@Autowired
	UtilsService Utils;
	
	// @GetMapping(value="/vkc/filter")
		@Description(value = "Loading  API Consume")
		public List<MrpTypePropertiesDAO> getmrpType()
				throws UnirestException, JsonMappingException, JsonProcessingException {

			HttpResponse<String> jsonresponse = Utils.ApiCall(
					"https://" + Utils.port + "-" + APIConstants.masterReport.YY1_MRPTYPE,
					Utils.apiUserName, Utils.apiPassword);
			JsonNode getJsonValues = Utils.XmlToJsonConversion(jsonresponse.getBody());
			ObjectMapper objectMapper = new ObjectMapper();

			List<MrpTypeDAO> entryNodesValues = objectMapper.reader()
					.forType(objectMapper.getTypeFactory().constructCollectionType(List.class,MrpTypeDAO.class))
					.readValue(getJsonValues.toString());

			List<MrpTypePropertiesDAO> customerList = entryNodesValues.stream()
