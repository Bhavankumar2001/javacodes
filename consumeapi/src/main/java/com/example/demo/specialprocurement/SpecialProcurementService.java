...
@Service
public class SpecialProcurementService {
	@Autowired
	UtilsService Utils;
	
	// @GetMapping(value="/vkc/filter")
		@Description(value = "Loading  API Consume")
		public List<SpecialProcurementPropertiesDAO> getspecialprocurement()
				throws UnirestException, JsonMappingException, JsonProcessingException {

			HttpResponse<String> jsonresponse = Utils.ApiCall(
					"https://" + Utils.port + "-" + APIConstants.masterReport.YY1_SPECIALPROCUREMENTTYPE,
					Utils.apiUserName, Utils.apiPassword);
			JsonNode getJsonValues = Utils.XmlToJsonConversion(jsonresponse.getBody());
			ObjectMapper objectMapper = new ObjectMapper();

			List<SpecialProcurementDAO> entryNodesValues = objectMapper.reader()
					.forType(objectMapper.getTypeFactory().constructCollectionType(List.class,SpecialProcurementDAO.class))
					.readValue(getJsonValues.toString());

			List<SpecialProcurementPropertiesDAO> customerList = entryNodesValues.stream()
