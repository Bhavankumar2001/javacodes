...
//@Service
@RestController
public class StockdeterminationService {
	...
	public List<StockdeterminationPropertiesDAO> getstockdetermination()
			throws UnirestException, JsonMappingException, JsonProcessingException {

		HttpResponse<String> jsonresponse = Utils.ApiCall(
				"https://" + Utils.port + "-" + APIConstants.masterReport.YY1_STOCKDETERMINATIONGROU, Utils.apiUserName,
				Utils.apiPassword);
		JsonNode getJsonValues = Utils.XmlToJsonConversion(jsonresponse.getBody());
		ObjectMapper objectMapper = new ObjectMapper();

//	
		List<StockdeterminationDAO> entryNodesValues = objectMapper.reader()
				.forType(objectMapper.getTypeFactory().constructCollectionType(List.class, StockdeterminationDAO.class))
				.readValue(getJsonValues.toString());

		List<StockdeterminationPropertiesDAO> customerList = entryNodesValues.stream()
